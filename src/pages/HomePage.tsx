...
const HomePage = () => {
  ...
  if (loading) return <div className="text-center p-20 text-gray-400">Загрузка ленты...</div>;

  return (
    <div className="space-y-8">
      
      {/* Наш новый чистый компонент поиска */}
      <SearchFilter 
      ...
      {filteredTrophies.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTrophies.map((trophy) => (
             <Link to={`/trophy/${trophy.id}`} key={trophy.id} className="group">
                 <div className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-gray-100 flex flex-col h-full">
                     ...
                     <div className="p-5 flex items-center justify-between bg-white border-t border-gray-50">
                        <div className="flex-1">
                          {!trophy.isNotFormat ? (
                            <div className="inline-flex items-center border-[2.5px] border-gray-800 rounded-lg px-2 py-0.5 font-mono transform group-hover:scale-105 transition-transform">
                              <span className="text-2xl font-black text-gray-800">{trophy.number}</span>
                              <span className="ml-2 pl-2 border-l border-gray-800 text-sm font-bold text-gray-800">{trophy.region}</span>
                            </div>
                          ) : (
                            <div className="text-sm font-mono font-black text-blue-600 truncate max-w-[120px]">
        ...
        <div className="py-20 text-center">
          ...
          <p className="text-gray-400">По таким параметрам ничего не найдено</p>
        </div>
      )}
    </div>
  );
